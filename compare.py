...
def run(argv):
    if len(argv) != 4:
        ...
        spreadsheetURL = 'https://docs.google.com/spreadsheets/d/1rvOMtucwaM5kD8j4GOxpHAMQnh0U064GMv8oUazq4Kw/edit#gid=772342231'
        spreadsheetId = spreadsheetURL.split('/')[-2]

        arguments = {"custId"           : 2355,
                     "preId"            : 13854277,
                     ...
                     'ignoredProps'     : ['FILEDATE', 'FILE_PREFIX', 'FILE_NAME', 'SIG_BMP', 'XML_DATA', 'BT_PRINT_FILE_NAME', 'BILLING_ADDRESS_BEG1', 'BILLING_ADDRESS_BEG2',
    ...
    master_mysqlClient = InitSQLClient(master=True)
    dstack = {}
    for k,v in master_mysqlClient.items():
        ...
        cursor.execute("SELECT r.Schema FROM billingmaster.fsimastercustomer mc "                     \
                       ...
                       "WHERE mc.customerid = %s;" % arguments['custId'])
        schema = cursor.fetchone()
        dstack[k] = schema[0]
        cursor.close()

    mysqlClient = InitSQLClient(dstack)
